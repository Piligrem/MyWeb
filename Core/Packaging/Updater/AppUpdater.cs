...
using System.IO;
using Log = InSearch.Core.Logging;
using NuGet;
...
namespace InSearch.Core.Packaging
{
    ...
    internal sealed class AppUpdater : DisposableObject
    {
        ...
        {
            // NEVER EVER (!!!) make an attempt to auto-update in a development environment!!!!!!!
        ...
        private IPackage FindPackage(out string path)
        {
            ...
            var files = Directory.GetFiles(dir, "InSearch.*.nupkg", SearchOption.TopDirectoryOnly);
        ...
        {
            if (package.Id != "InSearch")
                return false;

            var currentVersion = new SemanticVersion(InSearchVersion.FullVersion);
        ...
        private void Backup()
        {
            ...
            var tempPath = CommonHelper.MapPath("~/App_Data/_Backup/App/InSearch");
            ...
            {
                var exception = new InSearchException("Too many backups in '{0}'.".FormatInvariant(tempPath));
