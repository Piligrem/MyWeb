...
(
	...
	@AllowedUserRoleIds	nvarchar(MAX) = null,	--a list of customer role IDs (comma-separated list) for which a product should be shown (if a subjet to ACL)
	@PageIndex			int = 0, 
...
BEGIN
	...
	SELECT CAST(data as int) FROM [sm_splitstring_to_table](@AllowedUserRoleIds, ',')
...
BEGIN
	--create catalog
	EXEC('
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE [name] = ''InSearchFullTextCatalog'')
		CREATE FULLTEXT CATALOG [InSearchFullTextCatalog] AS DEFAULT')
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[Product]''))
		...
		KEY INDEX [' + dbo.[sm_getprimarykey_indexname] ('Product') +  '] ON [InSearchFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[ProductVariantAttributeCombination]''))
		...
		KEY INDEX [' + dbo.[sm_getprimarykey_indexname] ('ProductVariantAttributeCombination') +  '] ON [InSearchFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[LocalizedProperty]''))
		...
		KEY INDEX [' + dbo.[sm_getprimarykey_indexname] ('LocalizedProperty') +  '] ON [InSearchFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
	...
	IF NOT EXISTS (SELECT 1 FROM sys.fulltext_indexes WHERE object_id = object_id(''[ProductTag]''))
		...
		KEY INDEX [' + dbo.[sm_getprimarykey_indexname] ('ProductTag') +  '] ON [InSearchFullTextCatalog] WITH CHANGE_TRACKING AUTO'
	EXEC(@create_index_text)
END
GO
...
BEGIN
	...
	--drop catalog
	EXEC('
	IF EXISTS (SELECT 1 FROM sys.fulltext_catalogs WHERE [name] = ''InSearchFullTextCatalog'')
		DROP FULLTEXT CATALOG [InSearchFullTextCatalog]
	')
END
GO
