...
namespace InSearch.Services.Localization
{
    ...
    {
        #region Constants
        private const string LOCALESTRINGRESOURCES_ALL_KEY = "InSearch.lsr.all-{0}";
        private const string LOCALESTRINGRESOURCES_PATTERN_KEY = "InSearch.lsr.";
        #endregion Constants
        ...
        private class LocaleStringResourceParent : LocaleStringResource
        {
            public LocaleStringResourceParent(XmlNode localStringResource, string nameSpace = "")
            {
                ...
                {
                    throw new InSearchException("All language resources must have an attribute Name=\"Value\".");
                }
                var resName = resNameAttribute.Value.Trim();
                if (string.IsNullOrEmpty(resName))
                {
                    throw new InSearchException("All languages resource attributes 'Name' must have a value.'");
                }
